...
sub create_look_ups {
    my $dbh = shift or make_dbh();

    my $look_ups_sheet = $workbook->add_worksheet("lookups");
    $look_ups_sheet->protect("password");

    my $look_up_count = 0;
    foreach my $name (registered_look_ups()) {
	my $proc = find_look_up($name);
...
sub create_sheet {
    ...
    while (my ($col, $field_name) = each @{ table_info($table)->{_field_order} }) {
...
sub add_column {
    ...
    my $field_info = table_info($table)->{$field_name};
...
sub ingest_workbook {
	...
	if (grep { $_ eq $arg; } table_order()) { push @exclude_tables, $arg; }
    }
    if (not defined $dbh) { $dbh = make_dbh(); }
...
sub ingest_worksheet {
    ...
    # order
    while (my ($col, $head) = each @{ table_info($table)->{_field_order} }) {
	...
	while (my ($col, $field_name) = each @{ table_info($table)->{_field_order} }) {
	    ...
	    if (defined $cell) {
		if (is_look_up($table, $field_name)) { $record{$field_name} = parse_look_up_value($cell->value()) || $cell->value(); }
		else { $record{$field_name} = $cell->value(); }
	    } else { $record{$field_name} = undef; }
	}

	#if (record_empty($table, \%record)) {
	#    print "| | $row is EMPTY, so stopping:\n" . Dumper(\%record) . "\n";
	#} else {
	#    print "| | $row is NOT empty:\n" . Dumper(\%record) . "\n";
	#}

	last if (record_empty($table, \%record));

	# update or insert the record
	if (record_different($table, \%record)) {
	    update_record(($table, \%record));
	    print " Updated $table " . Dumper(\%record) . "\n";
	} elsif (not record_exists($table, \%record)) {
	    insert_record(($table, \%record, {insert_all_fields => 1}));
